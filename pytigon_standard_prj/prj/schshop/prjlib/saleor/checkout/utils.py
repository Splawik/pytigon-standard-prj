...
#SCH++
#from big_company.product import mod_chckout_queryset
...
def get_user_checkout(
    user: User, checkout_queryset=None, auto_create=False
...
# SCH++        
        ...
        #    },
        #)
        #return mod_chckout_queryset(checkout_queryset).get_or_create(
        #    user=user,
...
# SCH++        
    ...
    #return mod_chckout_queryset(checkout_queryset.filter(user=user)).first(), False

    x = checkout_queryset.filter(profile_str=profile_str).first()    
    return x, False
# SCH--


def get_anonymous_checkout_from_token(token, checkout_queryset=None):
...
def get_or_create_checkout_from_request(
    request, checkout_queryset=None
) -> Checkout:
...
def get_checkout_from_request(request, checkout_queryset=None):
...
#SCH++
        ...
        return Checkout(user=user, profile_str=get_profile_id())
#SCH--        
    #return Checkout()
    return None


def get_or_empty_db_checkout(checkout_queryset=None):
...
def prepare_order_data(*, checkout: Checkout, tracking_code: str, discounts) -> dict:
    ...
    order_data['total'].gross.amount = order_data['total'].net.amount * Decimal("1.23")
    order_data['total_price_left'] = order_data['total'].gross
...
# SCH++
    order.profile_str = checkout.profile_str
    order.save()
# SCH--
