...
def summary_without_shipping(request, checkout):
    ...
    if request.POST:
        ...
        #SCH++
        il_pal = checkout.lines.exclude(variant__sku = 'paleta').aggregate(Sum('quantity'))['quantity__sum']
        x = 0
        objects = checkout.lines.filter(variant__sku = 'paleta')        
        if len(objects)>0:
            x = objects[0].quantity
        if x != il_pal:
            x = ProductVariant.objects.filter(sku = 'paleta')
            if len(x) > 0:
                add_variant_to_checkout(checkout, x[0], il_pal, True)
        #SCH--

    ctx = get_checkout_context(checkout, request.discounts)
    
    #SCH++
    ctx['checkout_total'].gross.amount  = ctx['checkout_total'].net.amount * Decimal("1.23")
    ctx['checkout_subtotal'].gross.amount = ctx['checkout_subtotal'].net.amount * Decimal("1.23")
    #SCH--
