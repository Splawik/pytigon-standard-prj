...
class Order(ModelWithMetadata, AbstractOrder):
#class Order(AbstractOrder):

    def get_absolute_url(self):
        return settings.SHOP_URL + reverse("order:details", kwargs={"token": self.token})

    def get_total_amount(self):
        return self.total_gross_amount

    def get_buyer_info(self):
        ret = { "email": self.get_customer_email() }
        if ':' in self.user.first_name:
            x = self.user.first_name.split(':')
            logo = x[0].strip()
            first_name = x[1].strip()
            if logo:
                ret['customerId'] = logo            
            if first_name:
                ret['firstName'] = first_name
        else:
            ret['firstName'] = self.user.first_name
        if self.user.last_name:
            ret['lastName'] = self.user.last_name
        
        return ret
        
        #return { "email": self.get_customer_email() }
    ...
    def get_description(self):
        ...
        return "Zamówienie nr %d" % self.pk
...
#SCH--
    ...
    def get_last_payment(self):
        ...
        return max(self.payments.all(), default=None, key=attrgetter("created"))
        #SCH--        
...
class OrderLine(models.Model):
    ...
    def get_total(self):
        ...
        ret = self.unit_price * self.quantity
        ret.gross.amount = ret.net.amount * Decimal("1.23")
        return ret
        #SCH--
