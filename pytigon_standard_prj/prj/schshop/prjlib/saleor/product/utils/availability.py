...
def get_product_availability_status(product):
    ...
    are_all_variants_in_stock = all(
        variant.is_in_stock() for variant in product.variants2.all()
    )
    is_in_stock = any(variant.is_in_stock() for variant in product.variants2.all())
    requires_variants = product.product_type.has_variants

    if not product.is_published:
        return ProductAvailabilityStatus.NOT_PUBLISHED
    if requires_variants and not product.variants2.exists():
...
def _get_total_discount(
    ...
    """
    if not isinstance(undiscounted, (TaxedMoneyRange,)):
