...
# SCH++
#from big_company.product import ProductsQueryset_as_manager, ProductVariantQueryset_as_manager, base_price
#from big_company.product import ProductProxy, ProductVariantProxy, base_price
from big_company.tools import base_price
...
#SCH++    
    ...
    #objects = ProductsQueryset_as_manager(ProductsQueryset)
    #objects = ProductProxy(ProductsQueryset.as_manager())
    #print("?????????????????????????????????????????1")
    #print(type(objects))
    #print("?????????????????????????????????????????1")
    ...
    def variants2(self):
        p = get_profile()
        if p:
            return self.variants.filter(Q(quantity__gt = 0, param1 = p['magazyn'])|Q(sku='paleta'))
...
#SCH--
    ...
    #    #self.variants = ProductProxy(self.variants)
    ...
    def __iter__(self):
        ...
        #SCH--
        setattr(self, "__variants", self.variants2.all())
    ...
    def get_item_price(self):
        p_min = p_max = None
        unit = ""
        for v in self.variants2.all():
            price = Decimal(float(v.get_price().amount) / v.converter)
            unit = v.unit2
            if p_max == None or price > p_max:
                p_max = price
            if  p_min == None or price < p_min:
                p_min = price
        return (p_min, p_max, unit)
    #SCH--

    #SCH++
    def get_item_converter(self):
        p_min = p_max = None
        unit = ""
        for v in self.variants2.all():
            c = Decimal(v.converter)
            unit = v.unit2
            if p_max == None or c > p_max:
                p_max = c
            if  p_min == None or c < p_min:
                p_min = c
        return (p_min, p_max, unit)
    #SCH--
...
#SCH++
    unit2 = models.CharField(max_length=32, blank=True, null=True)    
    weight2 = models.FloatField(blank=True, null=True)    
    converter = models.FloatField(blank=True, null=True)    
    quantity2 = models.IntegerField(validators=[MinValueValidator(0)], default=Decimal(1), blank=True, null=True)        
    price2 = MoneyField(
    ...
    #objects = ProductVariantQueryset_as_manager(ProductVariantQueryset)
    #objects = ProductVariantProxy(ProductVariantQueryset.as_manager())
#SCH--
