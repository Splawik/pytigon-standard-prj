...
def map_status(status):
    s = { 
        "new": "not-charged",
        "prepared": "not-charged",
        "pre-auth": "not-charged",
        "charge_started": "not-charged",
        "partially_paid": "partially-charged",
        "paid": "fully-charged",
        "failed": "not-charged",
        "refund_started": "not-charged",
...
class Payment(AbstractPayment, models.Model):    
#SCH--
...
#SCH++
    ...
    def save(self, *argi, **argv):
        self.charge_status=map_status(self.status)
        if self.created_on:
            self.created = self.created_on
        if self.last_payment_on:
            self.modified = self.last_payment_on
        
        self.captured_amount = self.amount_paid
    ...
    @transition(field=status, source=[ps.PAID,ps.REFUND_STARTED], target=ps.REFUNDED)
    def confirm_refund(
        ...
        self.amount_refunded += Decimal(amount)
...
#SCH++
    ...
    #@property
    #def modified(self):
    #    return self.last_payment_on

    #@modified.setter
    #def modified(self, value):
    #    self.last_payment_on = value
#SCH--
