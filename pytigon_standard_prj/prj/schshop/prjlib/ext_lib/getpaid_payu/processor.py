...
class PaymentProcessor(BaseProcessor):
    ...
    def get_paywall_context(self, request=None, camelize_keys=False, **kwargs):
        ...
        context = {
            ...
            "products": products,            
            'continueUrl': self.payment.order.get_absolute_url(),
        ...
        if expected_signature == signature:
            ...
            if "order" in data:
                ...
                if status == RefundStatus.FINALIZED:
                    amount = float(refund_data.get("amount")) / 100
    ...
    def prepare_lock(self, request=None, **kwargs):
        ...
        logging.error("prepare_lock:")
        logging.error(params)
        response = self.client.new_order(**params)
        logging.error("prepare_lock_2:")
        logging.error(self.client.last_response)
        results["raw_response"] = self.client.last_response
