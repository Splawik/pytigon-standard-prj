...
class Client:
    ...
    def _authorize(self):
        ...
        if self.last_response.status_code == 200:
            ...
            #logging.error("============== _authorize ==================")
            #logging.error(url)
            #logging.error(self.oauth_id)
            #logging.error(self.oauth_secret)
            #logging.error("============================================")
        ...
        **kwargs,
        ...
        data = {
            ...
            if products
                ...
                {
                    "name": "Total order",
                    "unitPrice": float(self._convert(amount)),
                    "quantity": 1,
                }
            ],
        }
        if notify_url:
            ...
            data["continueUrl"] = notify_url
        if buyer:
            data["buyer"] = buyer
        if 'continueUrl' in kwargs:
            data['continueUrl']  = kwargs['continueUrl']
        #if 'buyer' in kwargs:
        headers = self._headers(**kwargs)
        data.update(kwargs)
        encoded = json.dumps(data, cls=DjangoJSONEncoder)
        logging.error("new_order:")
        logging.error(url)
        ...
        logging.error(self.last_response.text)
        logging.error("/new_order") 
