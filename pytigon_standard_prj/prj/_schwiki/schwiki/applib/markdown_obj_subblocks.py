    ...
    def gen_context(self, param, lines, output_format, parent_processor):
        if lines:
            ihtml_processor = IHtmlProcessor(
                output_format="html", parent_processor=parent_processor
            )
            return {
                "content": ihtml_processor.convert("\n".join(lines)),
                "param": param,
            }
    ...
    def gen_context(self, param, lines, output_format, parent_processor):
        if lines:
            i_markdown_processor = IndentMarkdownProcessor(
                output_format=output_format, parent_processor=parent_processor
            )
            return {
                "content": i_markdown_processor.convert("\n".join(lines)),
                "param": param,
            }
        return {"content": "", "param": param}
...
class DivObjRenderer(BaseMarkdownObjRenderer):
    ...
    def render(self, param, lines, output_format, parent_processor):
        ...
        out = (
            ...
            + self.edit_on_page_link(parent_processor)
            + "</div><div "
...
class AjaxObjRenderer(BaseMarkdownObjRenderer):
    ...
    def render(self, param, lines, output_format, parent_processor):
        context = self.gen_context(param, lines, output_format, parent_processor)
        return (
            "<div class='ajax-region ajax-frame' data-region='wiki'>\n"
            + context["content"]
            + "\n</div>\n"
        )
...
class TableObjRenderer(BaseMarkdownObjRenderer):
    ...
    def render(self, param, lines, output_format, parent_processor):
        ...
        if context["param"]["narrow"]:
            out = context["content"].replace(
                "<table>", "<table class='%s' style='width:auto'>" % table_class
            )
        else:
            out = context["content"].replace(
        ...
        if context["param"]["responsive"]:
            out = (
                ...
                + self.edit_on_page_link(parent_processor, right=True)
                + out
                + "</div>"
            )
        else:
